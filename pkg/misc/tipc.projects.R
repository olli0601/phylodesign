...
{	
	...
	{							
		...
		sites						<- merge(sites, samples.seq, by="comid_old")
			...
			theta.model.Hx	<- lapply(seq_len(nrow(sites)),function(i)
							{					
								...
								{	
									...
									tmp			<- prj.popart.powercalc.by.acutelklratio.matchpa( abc.struct, tmp, hetclu.scale=1, return.top=25 )						
		...
		tpc.obs			<- lapply(seq_len(nrow(sites)),function(i)
				{					
					...
					if(!standalone)
					{
						argv		<<- prj.simudata.cmd(CODE.HOME, sites[i,comid_old], tmp[1,acute.H0], tmp[1,base.H0], nrep, sampling[1], sampling[2], cohort.dur, save=1, resume=1, verbose=1, debug.susc.const=0, debug.only.u=0)
						argv		<<- unlist(strsplit(argv,' '))		
						#print(argv)
						tpc			<- prj.simudata()					#print(tpc[["sum.attack"]]["Median"]); print(tpc[["sum.E2E"]]["Median"])										
						tpc.H0		<- tpc[["tpc.table.sample.median"]]
						
						argv		<<- prj.simudata.cmd(CODE.HOME, sites[i,comid_old], tmp[1,acute.H1], tmp[1,base.H1], nrep, sampling[1], sampling[2], cohort.dur, save=1, resume=1, verbose=1, debug.susc.const=0, debug.only.u=0)
						argv		<<- unlist(strsplit(argv,' '))					
						tpc			<- prj.simudata()
					...
					{
						#low acute
						cmd			<- prj.simudata.cmd(CODE.HOME, sites[i,comid_old], tmp[1,acute.H0], tmp[1,base.H0], nrep, sampling[1], sampling[2], cohort.dur, save=1, resume=1, verbose=1, debug.susc.const=0, debug.only.u=0)
						...
						#high acute
						cmd			<- prj.simudata.cmd(CODE.HOME, sites[i,comid_old], tmp[1,acute.H1], tmp[1,base.H1], nrep, sampling[1], sampling[2], cohort.dur, save=1, resume=1, verbose=1, debug.susc.const=0, debug.only.u=0)
						cmd			<- prj.hpcwrapper(cmd, hpc.walltime=hpc.walltime, hpc.mem=hpc.mem, hpc.load="module load R/2.15",hpc.nproc=1, hpc.q=hpc.q)
						cat(cmd)								
						signat		<- paste(strsplit(date(),split=' ')[[1]],collapse='_',sep='')
						outdir		<- paste(CODE.HOME,"misc",sep='/')
	...
	{			
		...
		lkl.theta	<- rbindlist( lapply( tmp, function(i)
						{					
							...
							abc.struct[,sample:= runif(nrow(abc.struct), min=as.numeric(sites[i,"%avg",with=F])-sites[i,sigma], max=as.numeric(sites[i,"%avg",with=F])+sites[i,sigma])]
							#abc.struct[,sample:= rnorm(nrow(abc.struct),mean=as.numeric(sites[i,"%avg",with=F]), sd=sites[i,sigma])]
							abc.struct								
						}) )
		#	add pool lkl.theta if there are pools in tpc.obs
		if(length(setdiff( names(tpc.obs), as.character(sites[,comid_old])) ))
		{
			...
			tmp			<- tmp[, 	{
										z	<- comid_old					
										subset(lkl.theta, comid_old==z, select=c(acute,base,Inc,E2E,sample))									
									},by="pool"]
			setnames(tmp, "pool", "comid_old")
			lkl.theta	<- rbind(lkl.theta, tmp, use.names=T)
		}		
		print(lkl.theta)
		if(remote)
		{
			...
			dummy			<- lapply(rev(names(tpc.obs)),function(x)
					{														
						#print( mlkl.theta[[i]] )
						#H0.H0
						cmd			<- prog.acute.loglkl.batch.cmd(CODE.HOME, paste(f.name.remote,".R",sep=''),	paste(f.name.remote,'_',x,"_H0",".R",sep=''),	x,	"H0")
						cat(cmd)
						cmd			<- prj.hpcwrapper(cmd, hpc.walltime=hpc.walltime, hpc.mem=hpc.mem, hpc.load="module load R/2.15",hpc.nproc=1, hpc.q=hpc.q)
						prj.hpccaller(paste(CODE.HOME,"misc",sep='/'), paste("phd_l0",paste(strsplit(date(),split=' ')[[1]],collapse='_',sep=''),"qsub",sep='.'), cmd)
						#H0.H1
						cmd			<- prog.acute.loglkl.batch.cmd(CODE.HOME, paste(f.name.remote,".R",sep=''),	paste(f.name.remote,'_',x,"_H1",".R",sep=''),	x,	"H1")
						cat(cmd)
						cmd			<- prj.hpcwrapper(cmd, hpc.walltime=hpc.walltime, hpc.mem=hpc.mem, hpc.load="module load R/2.15",hpc.nproc=1, hpc.q=hpc.q)
		...
		{				
			lkl.theta			<- lapply(names(tpc.obs),function(x)
					{							
						tmp			<- subset(lkl.theta, comid_old==x)
						if(verbose)	cat(paste("\nacute.loglkl.batch, process ",x,"\nnumber lkl evaluations: ",nrow(tmp)))						
						argv		<<- prog.acute.loglkl.batch.cmd(CODE.HOME, paste(f.name.remote,".R",sep=''),	paste(f.name.remote,'_',x,"_H0",".R",sep=''),	x,	"H0")
						argv		<<- unlist(strsplit(argv,' '))						
						ans			<- prog.acute.loglkl.batch()
						ans[, h:="H0"]
						argv		<<- prog.acute.loglkl.batch.cmd(CODE.HOME, paste(f.name.remote,".R",sep=''),	paste(f.name.remote,'_',x,"_H1",".R",sep=''),	x,	"H1")
						argv		<<- unlist(strsplit(argv,' '))						
			...
			lkl.theta			<- do.call("rbind",lkl.theta)		#rbindlist unstable for 880e3 entries!
					...
					lkl.e2e.site	<- lapply(scenarios, function(hyp)
							{
								...
								if(plot)				
								{
									...
									print(subset(df.true, comid_old==site & h==hyp))
...
prj.pipeline<- function()
{
	...
	if(0)
	{
		...
		dummy				<- prj.popart.powercalc.by.acutelklratio.lkl4Precomputed(sites, tpc.obs, cohort.dur=cohort.dur, f.name=f.name, resume=0, verbose=verbose, remote=1, remote.signat=remote.signat)
	...
	if(1)
	{
		...
		if(1)	#pessimistic
		{
			p.lab			<- 0.7*0.85											
			p.consent.coh	<- 0.9*0.9	
			opt.analysis.12	<- "pessimistic-SC12-1023"
			opt.analysis.45	<- "pessimistic-SC45-1023"			
		}			
		#		
		opt.sampling	<- "strue"
		#opt.sampling	<- "struefx10"
	...
	#
	#	evaluate lkl.e2e.div for various opt.sampling
	#
	if(1)
	{
		require(data.table)		
		...
		lkl.e2e.div		<- lapply(opt.samplings, function(opt.sampling)
				{
					...
					lkl.e2e.div[,sampling:=p.lab]
					lkl.e2e.div
				})	
		lkl.e2e.div		<- do.call("rbind",lkl.e2e.div)
		
		lkl.e2e.Ndeke	<- subset( lkl.e2e.div, comid_old=="Ndeke")
		lkl.e2e.Ngungu	<- subset( lkl.e2e.div, comid_old=="Ngungu")
		lkl.e2e.Maramba	<- subset( lkl.e2e.div, comid_old=="Maramba")
		lkl.e2e.meanAZA	<- subset( lkl.e2e.div, comid_old=="Maramba" | comid_old=="Ndeke")
		lkl.e2e.meanAZA	<- lkl.e2e.meanAZA[,  list(min.p=mean(min.p)) ,by=c("sampling","cmp")]
		
		#
		#	plot effect of sampling coverage
		#
		file			<- paste(dir.name,'/',"tpcdiv_",m.type,'_',opt.design,'_',opt.analysis,'_samplingeffect4mAZA_',p.lab,'_',p.consent.coh,".pdf",sep='')
		print(file)
		pdf(file, 4,4)
		par(mar=c(4,5.5,0.5,0.5))
		x							<- c(10,20,40,60,80,100)
		scenarios					<- c("S01","S02")
		cols						<- rep("#253494",2)	#rev( tail(brewer.pal(nrow(tpc.obs.Ndeke.sum)+2,"YlGnBu"),nrow(tpc.obs.Ndeke.sum)) )
		names(cols)					<- c("central","central")
		ltys						<- c(1,3)
		names(ltys)					<- c("S0 vs S1","S0 vs S2")
		plot(1,1, xlim=range(x), ylim=c(0,1), type='n', bty='n', xaxt='n',xlab="% sampling coverage", ylab="probability difference")
		axis(1, at=x)
		abline(h=0.8,lty=4)
		dummy	<- sapply(seq_along(scenarios), function(i)
				{
					lines(x, subset(lkl.e2e.meanAZA, cmp==scenarios[i])[,min.p], col=cols[1], type='b', lty=ltys[i])
				})		
		legend("bottomright", legend=names(ltys), lty=c(1,3), col=cols[1],bty='n', border=NA)		
		dev.off()
		stop()
		
	}
	#
	#	plot power for 40% 60% 80% on Ndeke and pool AZA
	#	
	if(0)
	{
		require(data.table)		
		...
		div.TownII.t			<- as.numeric(subset(div.TownII, select=min.p))
	...
	#
	#	plot H0 and H1 values from Anne s model in table
	#	
	if(0)
	{
		require(data.table)	
	...
	#
	#	plot tip cluster table for 1) Ndeke sampling 40% 60% 80%  100%
	#	
	if(0)
	{
		require(data.table)	
